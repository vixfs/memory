let timeForRembemer = 1 * 1000;   // (сек * 1000мс) Время для запоминания карт (от старта до закрытия)
let timeBeforeUnflip = 750;       // (мс) Время перед закрытием неверно найденных карт
let pointsForFound = 100;         // Кол-во очков за верную найденную пару
let pointsForFail = -50;           // Кол-во очков за ошибку

let sizeX;  // Кол-во столбцов
let sizeY;  // Кол-во строк
let gridSize;

let timeForGame = 10; // (сек) Таймер для прохождения уровня
let timeleft;
...
for (let i = 0; i < gridSize - 1; i++){
    if(cards[i] === cards[i+1]){
        if(cards[i+2] == null){
            ...
            [cards[i+1], cards[i+2]] = [cards[i+2], cards[i+1]];
        }
    }
}
...
function updateSizeGrid(){
    ...
    if(((window.innerHeight - 115) / sizeY) > ((window.innerWidth - 40) / sizeX)){
        $("#grid").css({
        ...
        $("#grid").css({
            "width": `calc((100vh - 115px) / ${sizeY} * ${sizeX})`,
            "grid-template-rows": `repeat( ${sizeY}, calc((100vh - 115px) / ${sizeY}) )`,
            "grid-template-columns": `repeat( ${sizeX}, calc((100vh - 115px) / ${sizeY}) )`,
        })
    }
};
